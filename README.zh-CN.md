...
- 恢复 Excel 内部浅色网格线，但保持云文档外框、阴影和拖拽装饰隐藏。
- 表格重新打开时回到左上角，并让脑图、表格高度随窗口自适应。
- 调整脑图提示和保存状态位置，避免遮挡左侧及右下角原生工具栏。

### v1.6.2

- 修复 NAS/LAN HTTP 环境中的 `digest` 报错，以及错误附件时间戳导致的同步异常。
- 移除手动保存按钮，保留自动写回、冲突确认和本机失败恢复。
- 修复连续编辑时恢复缓存可能被较早保存误删的问题。

### v1.6.1

- 附件写回后更新文档同步修订标记，使修改进入思源同步链路。

### v1.6.0

- Excel 与 FreeMind 脑图开始直接写回思源附件。
- 增加自动保存状态、本机失败恢复、跨页面/设备冲突检测和静态资源版本防缓存。
