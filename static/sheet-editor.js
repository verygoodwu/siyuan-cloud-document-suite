(() => {
  ...
  document.querySelector("#add-row").addEventListener("click", () => { model.sheets[active].data.push([]); scheduleSave(); renderGrid(); });
  document.querySelector("#add-col").addEventListener("click", () => { const sheet = model.sheets[active]; const cols = dimensions(sheet).cols; sheet.data.forEach((row) => { while (row.length <= cols) row.push(""); }); scheduleSave(); renderGrid(); });
